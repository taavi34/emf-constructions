...
export default function QuotePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setStatus('success')
        setForm({ name: '', email: '', phone: '', projectType: '', budget: '', location: '', message: '' })
      } else {
        setStatus('error')
      }
    } catch {
  ...
  return (
    <>
      {/* Header */}
      <section style={{ paddingTop: '140px', paddingBottom: '4rem', paddingLeft: '2rem', paddingRight: '2rem', textAlign: 'center', background: '#0a0a0a', position: 'relative' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '16px', marginBottom: '1rem' }}>
          ...
          <span style={{ fontFamily: 'var(--font-jost)', color: '#C9A84C', fontSize: '11px', letterSpacing: '0.4em', textTransform: 'uppercase' }}>Let's Talk</span>
          <span style={{ display: 'block', width: '40px', height: '1px', background: '#C9A84C' }} />
        </div>
        <h1 style={{ fontFamily: 'var(--font-cormorant)', fontSize: 'clamp(3rem, 7vw, 5rem)', fontWeight: '300', color: '#f0f0f0', marginBottom: '1rem' }}>
          Get a <span style={{ color: '#C9A84C', fontStyle: 'italic' }}>Quote</span>
        </h1>
        <p style={{ fontFamily: 'var(--font-jost)', color: '#555', fontSize: '14px', lineHeight: '1.9', fontWeight: '300', maxWidth: '480px', margin: '0 auto' }}>
          Share your vision with us. We'll get back to you within 24 hours with an honest assessment and next steps.
        </p>
      </section>

      {/* Form + Contact Info */}
      <section style={{ padding: '3rem 2rem 7rem', background: '#0a0a0a' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '4rem' }}>

          {/* Form */}
          <div>
            {status === 'success' ? (
              <div style={{ textAlign: 'center', padding: '4rem 2rem', border: '1px solid #C9A84C22', background: '#111' }}>
                <div style={{ fontFamily: 'var(--font-cormorant)', color: '#C9A84C', fontSize: '3rem', marginBottom: '1rem' }}>✦</div>
                <h3 style={{ fontFamily: 'var(--font-cormorant)', fontSize: '2rem', color: '#f0f0f0', marginBottom: '1rem', fontWeight: '300' }}>Thank You</h3>
                <p style={{ fontFamily: 'var(--font-jost)', color: '#555', fontSize: '13px', lineHeight: '1.8', fontWeight: '300' }}>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                  ...
                  <div>
                    ...
                    <input name="location" value={form.location} onChange={handleChange} placeholder="City, State" style={inputStyle} />
                  </div>
                </div>
                ...
                {status === 'error' && (
                  <p style={{ fontFamily: 'var(--font-jost)', color: '#c0392b', fontSize: '13px' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    color: '#0a0a0a',
                    background: status === 'loading' ? '#a07830' : '#C9A84C',
                    padding: '16px 32px',
                    border: 'none',
          ...
          <div style={{ paddingTop: '1rem' }}>
            <div style={{ marginBottom: '3rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '1.5rem' }}>
                ...
                <span style={{ fontFamily: 'var(--font-jost)', color: '#C9A84C', fontSize: '11px', letterSpacing: '0.3em', textTransform: 'uppercase' }}>Contact</span>
              </div>
              ...
              {[
                ...
                <div key={i} style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', alignItems: 'flex-start' }}>
                  ...
                  <div>
                    <div style={{ fontFamily: 'var(--font-jost)', fontSize: '10px', letterSpacing: '0.2em', textTransform: 'uppercase', color: '#444', marginBottom: '4px', fontWeight: '500' }}>{item.label}</div>
                    <div style={{ fontFamily: 'var(--font-jost)', color: '#888', fontSize: '14px', fontWeight: '300' }}>{item.value}</div>
                  </div>
                </div>
              ))}
            </div>

            <div style={{ padding: '2rem', border: '1px solid #C9A84C22', background: '#080808' }}>
              <div style={{ fontFamily: 'var(--font-cormorant)', color: '#C9A84C', fontSize: '1.3rem', fontStyle: 'italic', marginBottom: '0.8rem' }}>What to Expect</div>
              {['Response within 24 hours', 'Free initial consultation', 'Detailed project assessment', 'No obligation quote'].map((item, i) => (
                <div key={i} style={{ display: 'flex', gap: '12px', marginBottom: '0.7rem', alignItems: 'center' }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-jost)', color: '#555', fontSize: '13px', fontWeight: '300' }}>{item}</span>
                </div>
              ))}
            </div>
          </div>
